...
module.exports = {
    ...
    createUser: async (email, hashedPassword, name) => {
        try {
            ...
            const query2 = `INSERT INTO users(name, email, password, isverified, dateJoined, balance) VALUES('${name}', '${email}', '${hashedPassword}', false,'${Date.now()}', 100000)`;
            await client.query(query2);
    ...
    updateUserVerificationStatus: async userid => {
        try {
            ...
            const query2 = `UPDATE users SET isverified = true WHERE userid = ${userid}`;
            await client.query(query2);
            return { 
    ...
    returnCar: async (rentId, mileMeterEnd, gasConsumed, refundAmount) => {
        try {
            ...
            const result5 = await client.query(query5);
            if(parseInt(result5.rows[0].balance) < Math.floor((result3.rows[0].cartype === "AC" ? 1.5 : 1) * baseAmount)) {
                return {
                    status: "warning",
                    title: "warning",
                    info: "Out of money, recharge your money in profile page"
